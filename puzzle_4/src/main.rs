...
fn main() {
    ...
    for line in input_file_reader.lines() {
        ...
        if jobs_contains_full_overlap(&jobs) {
            total_sum += 1;
        }
    }
...
fn jobs_contains_full_overlap(jobs: &((i32, i32), (i32, i32))) -> bool {
    match jobs {
        ((elf_one_start, elf_one_end), (elf_two_start, elf_two_end))
            if 
                (elf_two_start <= elf_one_end &&
...
mod tests {
    mod part_one {
        use crate::{get_elves_jobs, jobs_contains_full_overlap};
        ...
        fn example_one() {
            ...
            assert_eq!(jobs_contains_full_overlap(&jobs), false);
        ...
        fn example_four() {
            ...
            assert_eq!(jobs_contains_full_overlap(&jobs), true);
